...
ensure_node_npm() {
  echo "[0/8] Checking for node & npm…"
...
ensure_electron_installed() {
  echo "[2/8] Ensuring Electron is installed (dev dependency)…"
...
fix_chrome_sandbox() {
  echo "[3/8] Fixing Electron chrome-sandbox (SUID helper)…"
  local SANDBOX_PATH=""
...
ensure_flatpak_tools() {
  echo "[4/8] Checking for flatpak + flatpak-builder…"
...
ensure_electron_builder() {
  echo "[5/8] Ensuring electron-builder exists…"
...
build_snap() {
  echo "[6/8] Building Snap…"
  npm run build-snap

  local SNAP_PATH=""
  SNAP_PATH="$(ls -1t dist/*.snap 2>/dev/null | head -n 1 || true)"
  [[ -n "${SNAP_PATH}" ]] || die "No .snap found in dist/ after build."
  ...
  read -r -p "Install snap now? [y/N] " yn
  case "${yn:-N}" in
    y|Y|yes|YES) need_sudo; sudo snap install "${SNAP_PATH}" --dangerous ;;
    *) echo "OK — not installing snap." ;;
  esac
}

build_flatpak() {
  echo "[6/8] Building Flatpak…"
...
echo "  2) Flatpak"
echo "  3) Both"
echo "  4) None (just prep/fix sandbox)"
echo
...
case "${choice}" in
  1) build_snap ;;
  2) build_flatpak ;;
  3) build_snap; build_flatpak ;;
  4) echo "Done (deps installed + chrome-sandbox fixed)." ;;
  *) die "Invalid choice." ;;
esac
